...
from config import share_num, share_round, headless

# 分享帖子
post_url = 'https://www.instagram.com/p/C3p4hWLPvq0/'
# 用户名文件
...
def run_group(cl):
    ...
    with sync_api.sync_playwright() as pw:
        ...
        for _ in range(share_num):
            ...
            button_text = '分享帖子'
            if button := page.locator(selector='div._abm0._abl_ > svg', has_text=button_text):
                try:
                    button.click(timeout=5000)
                    for _ in range(share_round): fill_user(next(users))
                    # 发消息
                    try:
                        if message_button := page.get_by_placeholder('输入消息...'):
